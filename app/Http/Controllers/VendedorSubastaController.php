    ...
    public function create(Request $request) {
        ...
        if (is_null($checkToken)) {
            if (!is_null($idSubasta) && !is_null($nameVendedor) && !is_null($companyVendedor) && !is_null($emailVendedor)
                ...
                $subasta->save();
                return response()->json(array('vendedorSubasta' => $subasta, 'status' => 'success', 'message' => 'Vendedor subasta creado', 'code' => 200), 200);
            } else {
                return response()->json(array('vendedorSubasta' => null, 'status' => 'error', 'code' => 400, 'message' => 'Faltan datos'), 200);
            }
        } else {
